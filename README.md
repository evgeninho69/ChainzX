...
Для деплоя проекта на хостинг Beget используйте подробную инструкцию:

- **[📦 Подробная инструкция по деплою](DEPLOY_DETAILED.md)** - пошаговое руководство с детальными объяснениями
- **[⚡ Быстрый старт](BEGET_QUICKSTART.md)** - краткая инструкция для опытных пользователей
- **[✅ Чеклист деплоя](CHECKLIST.md)** - список для проверки перед и после деплоя

### Основные шаги:

1. Подготовка проекта локально (`npm install && npm run build`)
2. Создание базы данных PostgreSQL в панели Beget
3. Загрузка проекта на сервер (через Git или FTP)
4. Настройка на сервере (зависимости, сборка, миграции)
5. Настройка Node.js приложения в панели Beget
6. Проверка работы (бот, админ-панель, API)

Подробности в файле `DEPLOY_DETAILED.md`.
